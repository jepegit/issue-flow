...
- **Budget:** `[issueflow].checks_watch_minutes` (default `15`), env
  `ISSUEFLOW_CHECKS_WATCH_MINUTES`. Baked into close/yolo at `issue-flow update`.
  Agent-enforced (`gh` has no max-duration flag). Non-positive values fall back
  to the default.
...
**Link.** Issue #172, `issue172_plan.md`.
