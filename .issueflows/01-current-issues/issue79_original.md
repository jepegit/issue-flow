...
A file has appeared in my users' `.cursor` folder. It is in the folder `skills-cursor/migrate-to-skill` and called `SKILL.md` and makes me wonder if cursor is adapting to a similar setup as Claude-Code (going away from` commands/*.md`). If this is true, we should modify issue-flow so that it follows the new convention.

Perform a web-search to find out if this is correct.
Then we need to figure out what to do.


This is the content of the `SKILL.md` file:
