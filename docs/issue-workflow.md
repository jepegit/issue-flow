...
| `/iflow` | `iflow.md` | **Smart dispatcher.** Detect current state and run `/issue-init`, `/issue-plan`, `/issue-start`, or `/issue-close` automatically. Never auto-dispatches to pick / pause / cleanup / yolo / fix / status / graphify. |
...
| `/issue-fix` | `issue-fix.md` | **Off-path.** Interactive iterative-fixes session: create one issue + long-lived branch, then loop over many small fixes (short plan each, recorded in `issue<N>_status.md`), ending with `/issue-close`. |
| `/issue-status` | `issue-status.md` | **Off-path, read-only.** Snapshot of where every issue stands — local tracking state (focus / parked / solved) plus open GitHub issues cross-referenced against it. Changes nothing. |
...
> On tools without project slash commands (e.g. Codex CLI), invoke the mirrored Agent Skills under `.cursor/skills/` instead — for example `issueflow-issue-init` in place of `/issue-init`.
...
| `issueflow-issue-fix` | `/issueflow-issue-fix` | Same flow as `/issue-fix`: set up an interactive iterative-fixes session, loop over small fixes, finish with `/issue-close`. Off-path. |
| `issueflow-issue-status` | `/issueflow-issue-status` | Same flow as `/issue-status`: read-only overview of focus / parked / solved issues plus open GitHub issues. Off-path; writes nothing. |
...
**Not auto-dispatched:** `/issue-pause`, `/issue-cleanup`, `/issue-yolo`, `/issue-fix`, and `/issue-status`. `/iflow` will mention them in its output when relevant (e.g. "after the PR merges, run `/issue-cleanup`") but never picks them for you.
...
## 9. `/issue-fix` — interactive iterative-fixes session
...
**What you pass:** An optional session name (used for the issue title and branch slug). No name → defaults to `iterative-small-fixes`. During an active session, a `/issue-fix <description>` (or just describing a fix) means "run the next fix".

**What the assistant does:**

1. **Set up (once).** Preflight (default branch, `git fetch --prune`, clean tree); create a GitHub issue with `gh issue create` (always, after confirmation) and capture `N`; create branch `<N>-<slug>` (off the default, or — when already on a non-default branch — ask whether to branch from current or default); delegate local capture to `/issue-init`; seed `issue<N>_status.md` with an unchecked `- [ ] Done` and an empty **`## Iterative fixes log`**.
...
3. **Finish.** Tells you to run `/issue-close` to land the session (it never auto-runs it); reminds you about `/issue-cleanup` after the PR merges.

**Coexists with `/issue-pick fix`:** that command is a one-shot setup back into the normal `/issue-plan` → `/issue-start` flow; `/issue-fix` stays and drives the loop until close.

**Off-path:** `/iflow` never auto-dispatches to `/issue-fix`. While a session is active, drive it with `/issue-fix` + `/issue-close`, not `/iflow`. GitHub only (`gh`); GitLab is not supported.

**Result:** A session issue + branch with a running fixes log, ready to land via `/issue-close`.

---

## 10. `/issue-status` — status overview of all issues (read-only)
...
**Off-path:** `/iflow` never auto-dispatches to `/issue-status`. It writes nothing, moves no files, and creates no branches, commits, or GitHub issues.
...
Detours:
  ...
  /issue-fix    — interactive session: one branch, many small fixes, then /issue-close
  /issue-status — read-only overview of all issues (focus / parked / solved + GitHub)
```
