...
| Command | File | Role |
|--------|------|------|
| `/iflow-pick` | `issue-pick.md` | **Front door.** Help choose the next issue (parked work first, else ranked open GitHub issues), create the branch, and run `/iflow-init`. Off-path; never auto-dispatched. |
...
| `/iflow-pause` | `issue-pause.md` | Park work safely: update status, move the issue group to `02-partly-solved-issues/`, optional WIP commit and branch switch. |
...
| `/iflow-cleanup` | `issue-cleanup.md` | Post-merge hygiene: switch to default, `git pull --ff-only`, `git fetch --prune`, delete merged local branches (single consolidated confirm). |
| `/iflow-yolo` | `issue-yolo.md` | All-in-one for small, low-risk issues: chains `init → plan → start → close` with up-front safeguards and a single confirmation. |
| `/iflow-fix` | `issue-fix.md` | **Off-path.** Interactive iterative-fixes session: create one issue + long-lived branch, then loop over many small fixes (short plan each, recorded in `issue<N>_status.md`), ending with `/iflow-close`. |
| `/iflow-status` | `issue-status.md` | **Off-path, read-only.** Snapshot of where every issue stands — local tracking state (focus / parked / solved) plus open GitHub issues cross-referenced against it. Changes nothing. |
| `/iflow-graphify` | `graphify.md` | **Off-path.** Rebuild the [graphify](https://iflow-graphify.net) knowledge graph (`graphify-out/graph.html`, `GRAPH_REPORT.md`, `graph.json`). Wraps `issue-flow graphify` / `graphify`. Optional: only meaningful when `graphifyy` is installed. |
...
| `iflow-version-bump` | `@iflow-version-bump` (often used from `/iflow-close`) | Bump `[project]` version in `pyproject.toml` via `uv version --bump patch|minor|major`. |
...
- `/iflow-close bump` or `/iflow-close patch` — bump **patch** (e.g. `1.2.0` → `1.2.1`) using `uv version --bump patch`.
- `/iflow-close bump minor` — bump **minor**.
- `/iflow-close bump major` or `/iflow-close major` — bump **major**.
...
Code + tests (+ status updates during work)
    │  /iflow-close  [optional: bump patch/minor/major]
    ▼
Commit → push → PR
    │
    │  (PR merges on GitHub)
