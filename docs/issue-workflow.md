...
6. **Sync with the default branch** — `issue-flow agent sync-branch --json` replays the issue branch onto `origin/<default>` so commits that landed while the issue was in flight are included instead of surfacing as a conflicted PR at merge time. Bookkeeping conflicts are resolved keep-both — `HISTORY.md` where both sides only added `## [Unreleased]` bullets (the in-flight issue's bullet last), and additive bullets / table rows in `.issueflows/04-designs-and-guides/*.md` or `issue<N>_status.md`; a branch stacked on a squash-landed parent is detected (`base_detected`, or `--base <ref>` explicitly) so only its own commits replay. Any other conflict aborts the rebase and stops close.
...
**Arguments:** Nothing (acts on the current branch), an explicit branch name, and/or a GitHub-audit token such as `include GitHub` / `with github` / `github`. Opt out of a baked-on Phase B with `no github` / `local only`. With `cleanup_include_github = true` in `config.toml`, Phase B runs by default without a token. Self-update tokens (`bleeding edge` / `no bleeding`) gate `issue-flow agent self-update` after a successful FF pull; with `on_bleeding_edge = true` that step runs by default. Trailing `all` (or `workspace`) runs cleanup for **every scaffolded workspace member** in one pass — surveyed by `issue-flow workspace cleanup --json`, with one Phase A1 confirm and one Phase A2 confirm for the whole workspace; members with a dirty product-code tree, detached HEAD, or no `origin` are skipped and reported.
...
3. **Phase A1** — `git switch <default>`, `git pull --ff-only`, `git fetch --prune`, and `git branch -d` on every `reachable` branch. If `-d` refuses, reports and moves on; it never escalates to `-D` here. If ff-only fails, run `issue-flow agent default-sync` and recover from its `action` — never rebase or force-push default. When self-update is enabled and the pull succeeded, run `issue-flow agent self-update --json` on home.
...
- optional: `onfail:stop|skip`, `nonyolo:merge|pr-only|stop`, `max:<n>`, `resume`, `parallel:<n>` (experimental)

**What it does:**

1. Expands aliases, then resolves the queue with `issue-flow agent queue … --json` (dependency order; closed issues skipped; blocked ones set aside; `yolo: true|false` per item plus a `nonyolo` summary).
2. Cap check (default max 10 without `max:<n>`).
3. Writes `cycle_status.md`, then for each issue runs the full chain from a clean default branch. **Yolo lane** (`yolo: true`): `/iflow-yolo` verbatim, PR auto-merged. **Non-yolo lane** (`yolo: false`): same chain and safeguards, landed per the `nonyolo` policy — `merge` (default `merge`; auto-merge like yolo), `pr-only` (open the PR, continue), or `stop` (halt before the issue). A `yolo: no` judgment is not a stop by itself.
...
5. Merge refusals from bookkeeping conflicts (`HISTORY.md` bullets, `04-designs-and-guides/*.md` bullets / table rows, `issue<N>_status.md`) and stacked-on-a-squash-landed-parent branches are resolved by `issue-flow agent sync-branch` (keep-both, `--base` auto-detect) and retried once; "already merged" is success.

**What it asks you:** **One consolidated confirm** (ordered queue, skipped/blocked, the non-yolo issues + their policy, auto-merge). Afterwards it stops only when input is strictly necessary: an unfixable failure, a refused merge the sync cannot fix, a spec that is ambiguous or contradictory (or not actually small, on the yolo lane), or anything outside the confirmed queue.
...
**Arguments:** Epic `<N>`, optional `stage <k>`, optional `loops:<n>`, optional `nonyolo:merge|pr-only|stop`, `review` (adversarial only), or `status` / `dry-run`.
...
3. Writes `auto_status.md` and runs `/iflow-cycle` for that stage — `yolo: no` issues take cycle's non-yolo lane (default policy `merge`) rather than stopping the run.
...
**When to use:** You have an **existing** GitHub issue `<N>` that should become an epic and you want the whole path hands-off: draft, publish every stage, `/iflow-auto` each epoch (yolo and non-yolo issues alike), a final review, then local cleanup (`-d` reachable, `-D` squash-landed) and `/iflow-status`.
...
4. Runs `/iflow-auto <N>` for each unfinished published stage, honouring `epoch_gated`. Issues the plan judged `yolo: no` run on cycle's **non-yolo lane** (policy `nonyolo:merge|pr-only|stop`, default `merge`) instead of stopping the drive. Pass `nonyolo:<policy>` to override for the run.
5. Final `/iflow-auto <N> review`; creates/reopens leftover findings. Does not start another epoch.
6. `/iflow-cleanup` `local only` with the `drive` token: `git branch -d` on reachable branches **and** `git branch -D` on squash-landed ones (tip SHAs printed; `unique_work` never touched; no Phase B). Then `/iflow-status`.
