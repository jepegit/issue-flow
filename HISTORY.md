...
- Iflow in epics. (#210)
- **Cleanup can finally prune squash-merged branches (#243).** `/iflow-cleanup` promised to delete branches "including squash-merges via `git cherry`" with `git branch -d`, but `-d` tests *reachability* while `git cherry` proves *patch equivalence* — so in a squash-merging repo it refused every landed branch and pruned nothing while reporting success. New read-only `issue-flow agent local-branches` classifies locals as `reachable` / `squash_landed` / `merged_pr_divergent` / `unique_work` / `skipped` with a tip SHA on each entry, and Phase A splits in two: `-d` on reachable branches under the old confirm, `-D` on provably-landed ones only behind a second confirm that lists names, tip SHAs, and the divergent branches' unique commits (recover with `git branch <name> <tip>`). A merged PR whose commits still differ counts as landed only when no unique commit is *newer* than the merge, so work pushed after a merge is never offered for deletion. Branches with unique work are never deletable, and the yolo/cycle/auto chains still never force-delete.
...
- **Guided setup for new and novice users (#246).** New `/iflow-setup` surface walks a first-time user from an empty folder to a working project. The read-only `issue-flow agent setup-status [--json]` reports the facts — `uv` / `git` / `gh` on `PATH`, whether the directory is its *own* git work tree (`git rev-parse` walks up, so a bare folder nested in another repo used to look initialized), GitHub auth, `pyproject.toml`, and scaffold presence — as an ordered blocker list with per-platform install hints; the agent then fixes them one at a time behind confirms, running `uv init` / `git init` / `gh repo create` when asked but never `gh auth login` or a tool install. New `--mode novice` installs only the guided lifecycle plus the safety nets (no hands-off, batch, or decomposition surfaces), seeds a confirm-everything `config.toml`, and implies `skill_level = "basic"`. Rendered rules are now membership-gated, so a restricted mode never advertises a command it did not install. Adds `docs/getting-started.md`.
- **Ops / no-PR path (#251).** New `/iflow-ops` and `/iflow-close ops` (`nopr`/`no-pr`) finish staging→prod-style work without a PR; `ops_label` (default `ops`) routes via `/iflow-pick` when `label_flows` is on (ops beats yolo). Product-code dirty trees and unique product commits are refused.
