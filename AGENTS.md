...
- **Assume `squash` merges on GitHub.** After a PR merges, **remind** the user to run **`/iflow-cleanup`** (switch to default, `git pull --ff-only`, `git fetch --prune`, `git branch -d` on merged locals under one confirm — never `-D`). Do **not** auto-run cleanup from close/yolo/cycle; `/iflow-close` no longer does this step itself.
