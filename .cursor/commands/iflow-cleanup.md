...
Run this after a PR has been merged (typically the one opened by `/iflow-close`). It detects the merge, switches back to the default branch, and — with a **single consolidated confirm** — deletes every local branch whose commits are already in the default branch (including squash-merged branches).

`/iflow-cleanup` is the only command in the workflow that touches local branches destructively (via `git branch -d`, never `-D`).
...
- A branch name — clean up that specific branch instead of the current one (e.g. `/iflow-cleanup 42-fix-login`).
...
3. **Check PR / merge state for the issue branch.**
   ...
   - **If the branch is not merged:**
     ...
     - Tell them to re-run `/iflow-cleanup` after the PR merges.
...
5. **Optional folder sweep** (safe; no destructive git).
   ...
   - Groups without a checked `Done` stay put — do not route them to `.issueflows/02-partly-solved-issues/` from `/iflow-cleanup`. That is `/iflow-pause`'s job and requires a human decision.
