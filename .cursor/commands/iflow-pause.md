...
- Short note — used as the "Remaining work" text verbatim (e.g. `/iflow-pause waiting on design input from @user`).
...
4. **Branch hygiene note (non-destructive).** Tell the user the issue branch (typically `<N>-<slug>`) is still on the remote / local list. `/iflow-pause` never deletes branches — resume with `/iflow-init <N>` (or just check out the branch) when ready to continue. The matching folder is now `.issueflows/02-partly-solved-issues/`.
...
- next step hint (e.g. "to resume: `/iflow-init <N>` will pull the group back into `.issueflows/01-current-issues/` after confirming the archived-issue guard")
...
- Do not open a PR. Do not bump the version. Those are `/iflow-close`.
