# Status overview of the repository's issues (`/issue-status`)

`/issue-status` prints a **read-only** snapshot of where every issue stands —
...
focus issue), `/issue-status` only reports and never changes anything.
...
2. **Focus issue** (`.issueflows/01-current-issues/`).
   ...
   - For the focus group, read the title from `issue<n>_original.md` and classify its **lifecycle stage** using the same file-presence logic as `/iflow` (first match wins):
     - **init** — no `issue<n>_original.md` yet → next step `/issue-init`.
     - **plan** — original exists, no `issue<n>_plan.md` → next step `/issue-plan`.
     - **start** — plan exists, status file missing or its `- [x] Done` is unchecked → next step `/issue-start`.
     - **close** — a status file contains `- [x] Done` (case-insensitive on `done`) → next step `/issue-close`.
   - Show the stage and the suggested next step.

3. **Parked work** (`.issueflows/02-partly-solved-issues/`).
   - List each `issue<n>_*` group: number, title (from `issue<n>_original.md`), and a one-line status if a status file exists. These are started-but-unfinished issues worth resuming (`/issue-pick` surfaces them first).
...
5. **Open GitHub issues** (skip entirely if input was `local`).
   ...
   - Cross-reference each open issue against the local folders and tag its **local state**:
     ...
     - **untracked** — no local group yet (candidate for `/issue-pick`).
...
- **Read-only.** `/issue-status` writes nothing, moves no files, and never creates branches, commits, or GitHub issues. It only reads `.issueflows/` and runs read-only `git` / `gh` queries.
- **Off-path.** Never auto-dispatch `/issue-status` from `/iflow`, `/issue-start`, or `/issue-close`. The user opts in.
...
- `/issue-status` — full report (local tracking + open GitHub issues).
- `/issue-status local` — local `.issueflows/` state only, no GitHub query.
- `/issue-status milestone v0.4` — full report, GitHub section biased toward the `v0.4` milestone.
