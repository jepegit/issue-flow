...
1.5 **Consult existing designs / guides.** If `.issueflows/04-designs-and-guides/this-project.md` exists, read it first for project-specific context. Then skim `.issueflows/04-designs-and-guides/` for design docs, design decisions, or documented good practices that touch the area you're about to plan. When relevant docs exist, cite them in the plan so the approach stays consistent with prior decisions.
