...
1. **Sanity check**
   ...
   - **Graph freshness (optional).** If this change touched the project's structure (new modules, big refactor, removed files) and `graphify-out/` exists, suggest the user run `/graphify` (default: `graphify update`, AST-only, no LLM key needed) before pushing so teammates pull a current `GRAPH_REPORT.md`. Do not run `/graphify` automatically — it is opt-in. Skip this bullet entirely if `graphify-out/` is not present.
