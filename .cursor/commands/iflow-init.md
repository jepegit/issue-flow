...
The text after this slash command is the **issue reference**. It may also be **empty or only whitespace** (user ran `/iflow-init` with no arguments).
...
1. Resolve the issue reference.
   - **A. No non-empty issue reference** (missing or whitespace-only after the command):
     ...
     - If the branch name is empty **or** equals `main` or `master` (**case-insensitive**): **stop**. Tell the user the issue cannot be inferred from this branch and they must run `/iflow-init` with an issue number, a full GitHub issue URL, or `owner/repo/#number`.
...
2a. **Triage comments** (only if the `comments` array is non-empty).
   - Follow the `iflow-comments` skill (`.cursor/skills/iflow-comments/SKILL.md`) for the rules; summary:
...
## Example invocations
- `/iflow-init 123`
- `/iflow-init https://github.com/owner/repo/issues/123`
- `/iflow-init owner/repo/#123`
- `/iflow-init` (no trailing text: on an issue-style branch like `123-fix-bug`, ask for confirmation then proceed as for issue `123`)
