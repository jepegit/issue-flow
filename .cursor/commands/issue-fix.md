# Interactive iterative-fix session (`/issue-fix`)

`/issue-fix` opens an **interactive working session** on a single long-lived branch for a stream of small, iterative fixes. You fix one small thing, use the code, find the next small thing, and so on — each fix gets a short plan and is implemented only if you want it, and every fix is recorded in the issue's markdown. Finish the whole session with `/issue-close`.
...
This is an **off-path** command — the lifecycle dispatcher (`/iflow`) never auto-runs it, because it creates GitHub issues and branches and then drives an open-ended loop. Invoke it explicitly. While a session is active, drive it with `/issue-fix` (continue fixing) and `/issue-close` (finish) — not `/iflow`.

## How it relates to other commands

- **`/issue-pick fix`** is a *one-shot* setup: it creates a general-fixes issue + branch and hands you back to the normal `/issue-plan` → `/issue-start` flow. `/issue-fix` instead **stays** with you and runs the iterative loop until you close. They coexist; pick `/issue-fix` when you want the ongoing session.
- **`/issue-close`** ends an `/issue-fix` session exactly like any other issue (tests, optional bump, status update, commit, push, PR).
...
- `/issue-fix polish-cli-output` — name the session `polish-cli-output`.
- `/issue-fix` — no name: default the slug to `iterative-small-fixes` (made unique by appending the new issue number, e.g. `iterative-small-fixes`/branch `<N>-iterative-small-fixes`).

After the session is set up, a bare `/issue-fix <description>` (or just describing a fix in chat) means "run the next fix in this session".
...
2. **Create the GitHub issue (always, with confirmation).** Show the proposed title and body, then create it with `gh issue create` (add `--repo owner/repo` if the default remote is ambiguous):
   ...
   - **Body** — note this is an interactive iterative-fixes session opened via `/issue-fix`; individual fixes are recorded in the issue's status markdown and landed together via `/issue-close`.
...
4. **Capture locally.** Delegate to the `/issue-init` flow for `<N>` (follow `.cursor/commands/issue-init.md`): fetch the issue, write `.issueflows/01-current-issues/issue<N>_original.md`, and run its archive sweep on the other groups. Do not re-implement that logic here.

5. **Seed the status file.** Create `.issueflows/01-current-issues/issue<N>_status.md` with:
   - a short header noting this is an interactive `/issue-fix` session,
   - an unchecked `- [ ] Done` checkbox (kept unchecked until `/issue-close`),
   - an **`## Iterative fixes log`** section (initially empty) where each fix is appended as a dated bullet.

## Phase 2 — the fix loop (repeat)

Each time the user proposes a small fix (or runs `/issue-fix <description>` during the session):
...
If a proposed "fix" turns out to be a substantial feature or touches many unrelated areas, say so and suggest handling it as its own issue (`/issue-init` → `/issue-plan` → `/issue-start`) rather than as a loop iteration.

## Phase 3 — finish

When the user is done fixing, tell them to run **`/issue-close`** to land the session (tests, optional version bump, status update, commit, push, PR). Do **not** auto-run it. After the PR merges, remind them to run `/issue-cleanup`.

## Constraints

- **Off-path.** Never auto-dispatch `/issue-fix` from `/iflow`, `/issue-start`, or `/issue-close`. The user opts in.
...
- **Keep the status checkbox accurate.** Leave `- [ ] Done` unchecked during the session; `/issue-close` flips it to `- [x] Done` when the work lands.
- **Delegate, don't duplicate.** Use `/issue-init` for local capture and `/issue-close` to finish, rather than re-implementing their logic here.
...
- the reminder to finish with `/issue-close`

## Example invocations

- `/issue-fix polish-cli-output` — set up the session (issue + branch + files), then start taking fixes.
- `/issue-fix` — set up a generic `iterative-small-fixes` session.
- `/issue-fix tighten the error message in cli.py` — run the next fix in an active session.
