# Rebuild the project's knowledge graph (`/iflow-graphify`)

`/iflow-graphify` rebuilds the [graphify](https://iflow-graphify.net) knowledge graph for this project so the assistant can navigate by graph instead of grepping through files. Outputs land in `graphify-out/` (`graph.html`, `GRAPH_REPORT.md`, `graph.json`).
...
See the [graphify CLI reference](https://iflow-graphify.net/iflow-graphify-cli-commands.html) for the full subcommand and flag list.
...
- Do **not** run `/iflow-graphify` automatically from `/iflow-start`, `/iflow-close`, or `/iflow`. The user opts in.
