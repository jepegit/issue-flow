...
Off-path commands (`/issue-pick`, `/issue-pause`, `/issue-cleanup`, `/issue-yolo`, `/issue-fix`, `/graphify`) are **not** auto-dispatched. Invoke them directly when you need them. (`/issue-pick` is the front door *before* `/issue-init` — use it when you have not chosen an issue yet. `/issue-fix` runs an interactive iterative-fixes session; while one is active, drive it with `/issue-fix` + `/issue-close`, not `/iflow`.)
...
3. **Report.** Summary should include:
   ...
   - a one-line hint when an **off-path** command is the natural next step, e.g.:
     ...
     - `graphify-out/GRAPH_REPORT.md` looks stale (large refactor, new modules) → "consider `/graphify` to refresh the graph"

## Constraints

- `/iflow` never skips a downstream command's own prompts. If the downstream step asks a question, surface it normally.
- `/iflow` never auto-dispatches to `/issue-pick`, `/issue-pause`, `/issue-cleanup`, `/issue-yolo`, `/issue-fix`, or `/graphify`. Those are explicit choices only.
