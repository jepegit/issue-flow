---
name: issueflow-issue-fix
description: >-
  Run the /issue-fix interactive session: set up one long-lived branch + GitHub
  ...
  issue<N>_status.md. Finish with /issue-close. Off-path: never auto-dispatched by
  /iflow. Always creates a GitHub issue (gh); GitLab is not supported.
disable-model-invocation: true
---

# issue-flow — interactive iterative-fix session (`/issue-fix`)

Follow this skill when the user wants an **ongoing working session** for many small fixes on one branch, rather than a single well-defined deliverable. Matches `.cursor/commands/issue-fix.md`.

## When to use

- The user runs `/issue-fix`, mentions "iterative fixes", "small fixes session", or "let's just fix things on a branch".
- They have a bucket of little improvements (small bugs, typos, chores, polish) to knock out together and land via one PR.

Do **not** use this skill from `/iflow`, `/issue-start`, or `/issue-close`. `/issue-fix` is explicit-only because it creates GitHub issues and branches and drives an open-ended loop. While a session is active, drive it with `/issue-fix` + `/issue-close`, not `/iflow`.

It **coexists** with `/issue-pick fix`: that command is a one-shot setup back into the normal `/issue-plan` → `/issue-start` flow, whereas `/issue-fix` stays and runs the loop until close.
...
4. **Capture locally.** Delegate to the `/issue-init` flow (or the `issueflow-issue-init` skill) for `<N>`: write `.issueflows/01-current-issues/issue<N>_original.md` and run its archive sweep. Do not duplicate that logic.
5. **Seed the status file.** Create `.issueflows/01-current-issues/issue<N>_status.md` with a short header (interactive `/issue-fix` session), an unchecked `- [ ] Done`, and an empty **`## Iterative fixes log`** section.
...
If a "fix" is really a substantial feature or sprawls across unrelated areas, say so and suggest handling it as its own issue via `/issue-init` → `/issue-plan` → `/issue-start`.

### Phase 3 — finish

Tell the user to run **`/issue-close`** to land the session (tests, optional bump, status update, commit, push, PR). Do not auto-run it. Remind them to run `/issue-cleanup` after the PR merges.
...
- Keep `- [ ] Done` unchecked during the session; `/issue-close` flips it.
- Delegate local capture to `/issue-init` and finishing to `/issue-close`; one fix per loop iteration, implemented only on explicit confirmation.
