...
- **Separate editor workspace per worktree (preferred).** After each `worktree add`, run `issue-flow agent open-workspace <worktree-path> --json` (print-only) and show the path. Ask once (or include in the cycle's consolidated confirm) before any `--open`. Never pass `--open` silently. If separate windows are unavailable, continue with worktree-only parallel — see `.issueflows/04-designs-and-guides/separate-workspaces.md`.
