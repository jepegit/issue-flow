...
Do **not** use this skill for `/issue-pick`, `/issue-pause`, `/issue-cleanup`, `/issue-yolo`, or `/issue-fix`. Those are explicit-only commands. (`/issue-pick` is the front door *before* `/issue-init`, for when no issue has been chosen yet. `/issue-fix` runs an interactive iterative-fixes session, driven by `/issue-fix` + `/issue-close`.)
...
- Never auto-dispatch to `/issue-pick`, `/issue-pause`, `/issue-cleanup`, `/issue-yolo`, or `/issue-fix`.
