---
name: issueflow-issue-status
description: >-
  Run the /issue-status report: a read-only snapshot of where every issue stands
...
# issue-flow — issue status overview (`/issue-status`)

Follow this skill when the user wants a bird's-eye view of every issue's status
rather than acting on the single focus issue. Matches `.cursor/commands/issue-status.md`.

## When to use

- The user runs `/issue-status`, mentions "status of issues", "what's the state of all issues", "where do things stand", or asks for an issue overview / dashboard.
- You want to see parked work, the focus issue's lifecycle stage, and open GitHub issues in one place.

Do **not** use this skill to *change* anything. It is read-only and off-path; for acting on the focus issue use `/iflow`, and to choose the next issue use `/issue-pick`.
...
2. **Focus issue** (`.issueflows/01-current-issues/`). For the focus group, read its title from `issue<n>_original.md` and classify the lifecycle stage with the `/iflow` first-match logic:
   - **init** — no `issue<n>_original.md` → `/issue-init`.
   - **plan** — original exists, no `issue<n>_plan.md` → `/issue-plan`.
   - **start** — plan exists, status missing or `- [x] Done` unchecked → `/issue-start`.
   - **close** — status contains `- [x] Done` (case-insensitive) → `/issue-close`.
