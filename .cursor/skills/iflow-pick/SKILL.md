...
3. `issue-flow agent open-workspace <path> --json` (print-only). Fold “create worktree + open window?” into the **existing** start confirm. Pass `--open` only after yes. Never auto `--open`.
4. Run `/iflow-capture` (and later plan/build/close) with `-C <worktree-path>`. Continue the session in that folder/window.
